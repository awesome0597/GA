...
DF = pd.DataFrame(
    columns=['Calls to fitness', 'Generation', 'Best Fitness', 'Best Individual', 'Word Percentage', 'local minimas'])
GENERATIONS = 200
...
class GeneticAlgorithm:
    ...
    def run(self):
        ...
        while self.generations < GENERATIONS and local_minima <= 10 and self.population[0][
            'word_percent'] < target_word_percentage:
        ...
        print(f"Calls to fitness per individual: {self.fitness_counter / self.population_size}")
        print(f"Calls to fitness: {self.fitness_counter}")
        new_data = {
            'Run Type': self.run_type,
            'Calls to fitness per individual': self.fitness_counter / self.population_size,
            'Calls to fitness': self.fitness_counter,
            'Generation': self.generations,
            'Best Fitness': self.population[0]['fitness'],
            'Best Individual': self.population[0],
            'Word Percentage': self.population[0]['word_percent'],
            'local minimas': num_local_minimas
        }
...
def main():
    for _ in range(30):
        ...
        algorithm = GeneticAlgorithm(population_size=1000, selection_rate=0.8, mutation_rate=0.25, run_type=RUN_TYPE,
                                     mutation_count=1)
        algorithm.run()
        print(_)  # Print the number of times the algorithm has run
    DF.to_csv('filename2.csv', index=False)
    for RUN_TYPE in range(1, 3):
        for _ in range(30):
            algorithm = GeneticAlgorithm(population_size=1000, selection_rate=0.8, mutation_rate=0.25,
                                         run_type=RUN_TYPE,
                                         mutation_count=5)
            algorithm.run()
            print(RUN_TYPE * _)  # Print the number of times the algorithm has run
    DF.to_csv('filename2.csv', index=False)
